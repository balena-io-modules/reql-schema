 ...
 */
const useSecondaryIndexes = (query, schema, options = {}) => {
	if (!options.indexes) {
		return query
	}
	const schemaProperties = schema.properties || {}

	// We do early returns, because we cannot chain multiple
	// getAll calls on secondary indexes
	for (const index of Object.keys(options.indexes)) {
		const idxQuery = schemaProperties[index] || {}
		if (typeof idxQuery.const !== 'undefined') {
			return query.getAll(idxQuery.const, {
				index
			})
		} else if (typeof idxQuery.enum !== 'undefined') {
			return query.getAll(...idxQuery.enum, {
				index
			})
		}
	}
...
module.exports = (rethinkdb) => {
	return (database, table, schema, options = {}) => {
		...
		const parseFragment = (fragment, name = []) => {
			if (fragment.type === 'object') {
				...
				if (fragment.not) {
					...
					const expression = rethinkdb.not(
						parseFragment(fragment.not, name))
					result = getConjunction(result, expression)
				}
		...
		query = useSecondaryIndexes(query, schema, options)
