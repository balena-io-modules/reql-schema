	...
	case 'type':
		if (Array.isArray(value)) {
			...
			for (const type of value) {
				const expression = row.typeOf().eq(getRethinkType(type))
		...
		return row.typeOf().eq(getRethinkType(value))
	case 'const':
		return row.eq(value)
	case 'pattern':
	...
	if (fragment.type === 'object') {
		...
		for (const property of Object.keys(fragment.properties)) {
			...
			// specifying the desired type (happens quite often)
				...
				let disjunction = row.typeOf().eq('NULL')

				for (const type of types) {
					const expression = row.typeOf().eq(getRethinkType(type))
					disjunction = getDisjunction(disjunction, expression)
				}

				result = getConjunction(result, disjunction)
			} else {
				const row = getRowFromProperty(path)
